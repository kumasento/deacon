...
public class ConvLayerManagerUtils {
  ...
  public static Map<String, KernelBlock> createKernelBlocks(
    ...
    for (ConvLayerParameters cp : cps) {
      ...
      KernelBlock knl = mgr.addKernel(new ConvLayerWrapKernel(
          mgr.makeKernelParameters(knlName), cp, numCoeffFifoSplits));
    ...
    if (useDRAM) {
      ...
      ofmapPadKnl.getInput(PaddingKernel.INP_NAME)
          .connect(knl.getOutput(OFMAP_NAME));
    } else {
  ...
  public static Map<String, KernelBlock> createPaddingKernels(
    ...
    for (int i = 0; i < cps.size(); i++) {
      ...
      if (cp.type == Type.STANDARD || cp.type == Type.DEPTHWISE_SEPARABLE_V2) {
        ...
        KernelBlock knl = mgr.addKernel(new UnpaddingKernel(
            mgr.makeKernelParameters(knlName),
            cp.getCoeffStreamBitWidth() / cp.getCoeffStreamChunkSize(),
            numCoeffFifoSplits, cp.dbg));
        knls.put(knlName, knl);
        ...
        KernelBlock depthwiseKnl = mgr.addKernel(new UnpaddingKernel(
            mgr.makeKernelParameters(depthwiseKnlName),
    ...
    LMemCommandGroup groupIfmap = iface.addCommandGroup(
        "GROUP_IFMAP", LMemCommandGroup.MemoryAccessPattern.LINEAR_1D);
    LMemCommandGroup groupOfmap = iface.addCommandGroup(
        "GROUP_OFMAP", LMemCommandGroup.MemoryAccessPattern.LINEAR_1D);
