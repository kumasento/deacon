...
public
class DepthwiseSeparableConvLayerKernelV2 extends ConvLayerKernel {

 public
  Map<String, BaseStream> streams;

 public
  static final String IFMAP = "ifmap";
 public
  static final String OFMAP = "ofmap";
 public
 ...
 public DepthwiseSeparableConvLayerKernelV2(KernelBase<?> owner, ConvLayerParameters cp, DFEType T) {
    ...
    streams[COEFF] =
        new BaseStream(COEFF, cp.PC * Math.max(cp.K * cp.K, cp.PF), T, coeffEn);
    ...
    // create ibuf
    ConvLayerIfmapBuffer ibuf = new ConvLayerIfmapBuffer(
        getOwner(), cp.createPointwiseParameters(), T, true, cp.name);
    ...
    ConvLayerOfmapBuffer obuf = new ConvLayerOfmapBuffer(
        getOwner(), cp.createPointwiseParameters(), T, cp.name + "_ofmap");
    obuf.setReset(getObufReset());
    ofmap.connect(
        obuf.port(pointOfmap, getObufAddr(obuf.getAddrT()), getObufWriteEn()));
 ...
 public
  void setIO(Kernel owner) {
    for (BaseStream stream : streams.values()) stream.setIO(owner);
  }

 private
  List<DFEVector<DFEVar>> splitMulsVec(DFEVector<DFEVar> muls) {
    List<DFEVector<DFEVar>> splits = new ArrayList<DFEVector<DFEVar>>();

    // for adder tree
    DFEVectorType<DFEVar> aT =
        new DFEVectorType<DFEVar>(T, cp.PC * cp.PK * cp.K * cp.K);
    ...
    for (int k = 0; k < cp.PF; k++) {
      for (int j = 0; j < cp.PK; j++) {
        DFEVector<DFEVar> tmp =
            (new DFEVectorType<DFEVar>(T, cp.PC)).newInstance(getOwner());
 ...
 private
  DFEVector<DFEVar> reduceByAdderTree(DFEVector<DFEVar> inp) {
    DFEVector<DFEVar> result =
        (new DFEVectorType<DFEVar>(T, cp.PC * cp.PK)).newInstance(getOwner());

    for (int i = 0; i < cp.PC * cp.PK; i++) {
      DFEVector<DFEVar> tmp =
          (new DFEVectorType<DFEVar>(T, cp.K * cp.K)).newInstance(getOwner());
 ...
 private
  void createCounters() {
    ...
    if (cp.dbg) debug.simPrintf("f = %d c = %d h = %d w = %d\n", f, c, h, w);
  }

 private
  DFEVector<DFEVar> fork(DFEVector<DFEVar> inp, int numCopies) {
    DFEVector<DFEVar> result =
        (new DFEVectorType<DFEVar>(T, numCopies * inp.getSize()))
            .newInstance(getOwner());
 ...
 private
  DFEVar getIbufAddr(DFEType addrT) {
 ...
 private
  DFEVar getObufAddr(DFEType addrT) {
    return (oh * cp.OW / cp.PK + ow).cast(addrT);
  }

 private
  DFEVar getObufWriteEn() { return (h >= cp.K - 1) & (w * cp.PK >= cp.K - 1); }

 private
  DFEVector<DFEVar> forkLbufOut(DFEVector<DFEVar> lbufOut) {
    DFEVector<DFEVar> result =
        (new DFEVectorType<DFEVar>(T, cp.PC * cp.PK * cp.K * cp.K))
            .newInstance(getOwner());
      ...
      for (int j = 0; j < cp.PK; j++) {
        for (int x = 0; x < cp.K; x++) {
          for (int y = 0; y < cp.K; y++) {
            int dstIdx =
                i * cp.PK * cp.K * cp.K + j * cp.K * cp.K + x * cp.K + y;
            int srcIdx = i * cp.K * (cp.K + cp.PK - 1) +
                         x * (cp.K + cp.PK - 1) + (j + y);
 ...
 private
  void connectMulsInput(DFEVector<DFEVar> mulsInp, DFEVector<DFEVar> lbufOut,
                        DFEVector<DFEVar> ibufOut) {
 ...
 private
  void connectMulsCoeff(DFEVector<DFEVar> mulsWgt, DFEVector<DFEVar> coeff) {
