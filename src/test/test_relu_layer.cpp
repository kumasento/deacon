...
// maxeler
#include "test_relu_layer_MaxDeep.h"
...
#define TEST_NAME "test_relu_layer"

TEST(SingleReLULayer, MainTest) {
  LOG(INFO) << TEST_NAME;

  std::vector<float> inp_data = read_test_data<float>(std::string(TEST_NAME), std::string("data"));
  std::vector<float> opt_data = read_test_data<float>(std::string(TEST_NAME), std::string("relu"));

  float *opt = (float *) malloc(sizeof(float) * opt_data.size());

  test_relu_layer_MaxDeep_actions_t actions;
  actions.instream_cpu_inp  = inp_data.data();
  actions.outstream_cpu_out = opt;

  max_file_t *max_file = test_relu_layer_MaxDeep_init();
  max_engine_t *engine = max_load(max_file, "local:*");

  LOG(INFO) << "Running test";
  test_relu_layer_MaxDeep_run(engine, &actions);
  LOG(INFO) << "Done!";
