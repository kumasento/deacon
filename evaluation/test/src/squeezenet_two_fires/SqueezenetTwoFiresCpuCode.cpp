...
struct Dfe {
  typedef SqueezenetTwoFires_actions_t dfe_run_actions_t;
  typedef SqueezenetTwoFires_dramRead_actions_t dram_read_actions_t;
  typedef SqueezenetTwoFires_dramWrite_actions_t dram_write_actions_t;

  static void ReadDRAM(max_engine_t *engine, dram_read_actions_t *actions) {
    SqueezenetTwoFires_dramRead_run(engine, actions);
  }
  static void WriteDRAM(max_engine_t *engine, dram_write_actions_t *actions) {
    SqueezenetTwoFires_dramWrite_run(engine, actions);
  }
};
...
int main(int argc, char *argv[]) {
  ...
  max_file_t *max_file = SqueezenetTwoFires_init();
  ...
  /* Launch run */
  SqueezenetTwoFires_actions_t actions;
  ...
  for (int i = 0; i < (int)num_iters; i++)
    SqueezenetTwoFires_run(engine, &actions);
