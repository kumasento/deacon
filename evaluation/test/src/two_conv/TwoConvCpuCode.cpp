...
struct Dfe {
  typedef TwoConv_actions_t dfe_run_actions_t;
  typedef TwoConv_dramRead_actions_t dram_read_actions_t;
  typedef TwoConv_dramWrite_actions_t dram_write_actions_t;

  static void ReadDRAM(max_engine_t *engine, dram_read_actions_t *actions) {
    TwoConv_dramRead_run(engine, actions);
  }
  static void WriteDRAM(max_engine_t *engine, dram_write_actions_t *actions) {
    TwoConv_dramWrite_run(engine, actions);
  }
};
...
int main(int argc, char *argv[]) {
  ...
  max_file_t *max_file = TwoConv_init();
  ...
  /* Launch run */
  TwoConv_actions_t actions;
  actions.param_batch_size = batch_size;

  LOG(INFO) << "Running ...\n";
  std::chrono::time_point<std::chrono::system_clock> start, end;
  start = std::chrono::system_clock::now();
  for (int i = 0; i < (int)num_iters; i++) TwoConv_run(engine, &actions);
