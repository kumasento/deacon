...
struct Dfe {
  typedef Resnet18FstId_actions_t dfe_run_actions_t;
  typedef Resnet18FstId_dramRead_actions_t dram_read_actions_t;
  typedef Resnet18FstId_dramWrite_actions_t dram_write_actions_t;

  static void ReadDRAM(max_engine_t *engine, dram_read_actions_t *actions) {
    Resnet18FstId_dramRead_run(engine, actions);
  }
  static void WriteDRAM(max_engine_t *engine, dram_write_actions_t *actions) {
    Resnet18FstId_dramWrite_run(engine, actions);
  }
};
...
int main(int argc, char *argv[]) {
  ...
  max_file_t *max_file = Resnet18FstId_init();
  ...
  std::vector<ConvLayerParameters> cps;
	...
	cps.push_back(ConvLayerParameters(max_file, "identity0"));
	cps.push_back(ConvLayerParameters(max_file, "identity1"));
  ...
  /* Launch run */
  Resnet18FstId_actions_t actions;
  ...
  for (int i = 0; i < (int)num_iters; i++)
    Resnet18FstId_run(engine, &actions);
