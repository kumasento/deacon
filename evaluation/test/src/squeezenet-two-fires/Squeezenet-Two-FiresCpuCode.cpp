...
struct Dfe {
  typedef Squeezenet-Two-Fires_actions_t dfe_run_actions_t;
  typedef Squeezenet-Two-Fires_dramRead_actions_t dram_read_actions_t;
  typedef Squeezenet-Two-Fires_dramWrite_actions_t dram_write_actions_t;

  static void ReadDRAM(max_engine_t *engine, dram_read_actions_t *actions) {
    Squeezenet-Two-Fires_dramRead_run(engine, actions);
  }
  static void WriteDRAM(max_engine_t *engine, dram_write_actions_t *actions) {
    Squeezenet-Two-Fires_dramWrite_run(engine, actions);
  }
};
...
int main(int argc, char *argv[]) {
  ...
  max_file_t *max_file = Squeezenet-Two-Fires_init();
  ...
  /* Launch run */
  Squeezenet-Two-Fires_actions_t actions;
  ...
  for (int i = 0; i < (int)num_iters; i++)
    Squeezenet-Two-Fires_run(engine, &actions);
