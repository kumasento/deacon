...
struct Dfe {
  typedef ThreeConvFff_actions_t dfe_run_actions_t;
  typedef ThreeConvFff_dramRead_actions_t dram_read_actions_t;
  typedef ThreeConvFff_dramWrite_actions_t dram_write_actions_t;

  static void ReadDRAM(max_engine_t *engine, dram_read_actions_t *actions) {
    ThreeConvFff_dramRead_run(engine, actions);
  }
  static void WriteDRAM(max_engine_t *engine, dram_write_actions_t *actions) {
    ThreeConvFff_dramWrite_run(engine, actions);
  }
};
...
int main(int argc, char *argv[]) {
  ...
  max_file_t *max_file = ThreeConvFff_init();
  ...
  /* Launch run */
  ThreeConvFff_actions_t actions;
  ...
  for (int i = 0; i < (int)num_iters; i++)
    ThreeConvFff_run(engine, &actions);
