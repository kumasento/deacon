...
struct Dfe {
  typedef SqueezenetFire_actions_t dfe_run_actions_t;
  typedef SqueezenetFire_dramRead_actions_t dram_read_actions_t;
  typedef SqueezenetFire_dramWrite_actions_t dram_write_actions_t;

  static void ReadDRAM(max_engine_t *engine, dram_read_actions_t *actions) {
    SqueezenetFire_dramRead_run(engine, actions);
  }
  static void WriteDRAM(max_engine_t *engine, dram_write_actions_t *actions) {
    SqueezenetFire_dramWrite_run(engine, actions);
  }
};
...
int main(int argc, char *argv[]) {
  ...
  max_file_t *max_file = SqueezenetFire_init();
  ...
  /* Launch run */
  SqueezenetFire_actions_t actions;
  ...
  for (int i = 0; i < (int)num_iters; i++)
    SqueezenetFire_run(engine, &actions);
