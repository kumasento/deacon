...
struct Dfe {
  typedef ThreeConvFfc_actions_t dfe_run_actions_t;
  typedef ThreeConvFfc_dramRead_actions_t dram_read_actions_t;
  typedef ThreeConvFfc_dramWrite_actions_t dram_write_actions_t;

  static void ReadDRAM(max_engine_t *engine, dram_read_actions_t *actions) {
    ThreeConvFfc_dramRead_run(engine, actions);
  }
  static void WriteDRAM(max_engine_t *engine, dram_write_actions_t *actions) {
    ThreeConvFfc_dramWrite_run(engine, actions);
  }
};
...
int main(int argc, char *argv[]) {
  ...
  max_file_t *max_file = ThreeConvFfc_init();
  ...
  /* Launch run */
  ThreeConvFfc_actions_t actions;
  ...
  for (int i = 0; i < (int)num_iters; i++)
    ThreeConvFfc_run(engine, &actions);
