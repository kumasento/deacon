...
struct Dfe {
  typedef TwoConvCfParFirst_actions_t dfe_run_actions_t;
  typedef TwoConvCfParFirst_dramRead_actions_t dram_read_actions_t;
  typedef TwoConvCfParFirst_dramWrite_actions_t dram_write_actions_t;

  static void ReadDRAM(max_engine_t *engine, dram_read_actions_t *actions) {
    TwoConvCfParFirst_dramRead_run(engine, actions);
  }
  static void WriteDRAM(max_engine_t *engine, dram_write_actions_t *actions) {
    TwoConvCfParFirst_dramWrite_run(engine, actions);
  }
};
...
int main(int argc, char *argv[]) {
  ...
  max_file_t *max_file = TwoConvCfParFirst_init();
  ...
  /* Launch run */
  TwoConvCfParFirst_actions_t actions;
  ...
  for (int i = 0; i < (int)num_iters; i++)
    TwoConvCfParFirst_run(engine, &actions);
