...
struct Dfe {
  typedef Invertedbottleneck_actions_t dfe_run_actions_t;
  typedef Invertedbottleneck_dramRead_actions_t dram_read_actions_t;
  typedef Invertedbottleneck_dramWrite_actions_t dram_write_actions_t;

  static void ReadDRAM(max_engine_t *engine, dram_read_actions_t *actions) {
    Invertedbottleneck_dramRead_run(engine, actions);
  }
  static void WriteDRAM(max_engine_t *engine, dram_write_actions_t *actions) {
    Invertedbottleneck_dramWrite_run(engine, actions);
  }
};
...
int main(int argc, char *argv[]) {
  ...
  max_file_t *max_file = Invertedbottleneck_init();
  ...
  /* Launch run */
  Invertedbottleneck_actions_t actions;
  ...
  for (int i = 0; i < (int)num_iters; i++)
    Invertedbottleneck_run(engine, &actions);
