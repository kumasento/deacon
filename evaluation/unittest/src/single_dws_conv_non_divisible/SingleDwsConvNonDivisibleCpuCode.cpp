...
struct Dfe {
  typedef SingleDwsConvNonDivisible_actions_t dfe_run_actions_t;
  typedef SingleDwsConvNonDivisible_dramRead_actions_t dram_read_actions_t;
  typedef SingleDwsConvNonDivisible_dramWrite_actions_t dram_write_actions_t;

  static void ReadDRAM(max_engine_t *engine, dram_read_actions_t *actions) {
    SingleDwsConvNonDivisible_dramRead_run(engine, actions);
  }
  static void WriteDRAM(max_engine_t *engine, dram_write_actions_t *actions) {
    SingleDwsConvNonDivisible_dramWrite_run(engine, actions);
  }
};
...
int main(int argc, char *argv[]) {
  ...
  max_file_t *max_file = SingleDwsConvNonDivisible_init();
  ...
  /* Launch run */
  SingleDwsConvNonDivisible_actions_t actions;
  ...
  for (int i = 0; i < (int)num_iters; i++)
    SingleDwsConvNonDivisible_run(engine, &actions);
