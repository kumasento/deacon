...
struct Dfe {
  typedef MobilenetV1ParS16_actions_t dfe_run_actions_t;
  typedef MobilenetV1ParS16_dramRead_actions_t dram_read_actions_t;
  typedef MobilenetV1ParS16_dramWrite_actions_t dram_write_actions_t;

  static void ReadDRAM(max_engine_t *engine, dram_read_actions_t *actions) {
    MobilenetV1ParS16_dramRead_run(engine, actions);
  }
  static void WriteDRAM(max_engine_t *engine, dram_write_actions_t *actions) {
    MobilenetV1ParS16_dramWrite_run(engine, actions);
  }
};
...
int main(int argc, char *argv[]) {
  ...
  max_file_t *max_file = MobilenetV1ParS16_init();
  ...
  /* Launch run */
  MobilenetV1ParS16_actions_t actions;
  ...
  for (int i = 0; i < (int)num_iters; i++)
    MobilenetV1ParS16_run(engine, &actions);
