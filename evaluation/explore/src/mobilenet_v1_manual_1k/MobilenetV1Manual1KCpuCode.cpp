...
struct Dfe {
  typedef MobilenetV1Manual1K_actions_t dfe_run_actions_t;
  typedef MobilenetV1Manual1K_dramRead_actions_t dram_read_actions_t;
  typedef MobilenetV1Manual1K_dramWrite_actions_t dram_write_actions_t;

  static void ReadDRAM(max_engine_t *engine, dram_read_actions_t *actions) {
    MobilenetV1Manual1K_dramRead_run(engine, actions);
  }
  static void WriteDRAM(max_engine_t *engine, dram_write_actions_t *actions) {
    MobilenetV1Manual1K_dramWrite_run(engine, actions);
  }
};
...
int main(int argc, char *argv[]) {
  ...
  max_file_t *max_file = MobilenetV1Manual1K_init();
  ...
  /* Launch run */
  MobilenetV1Manual1K_actions_t actions;
  ...
  for (int i = 0; i < (int)num_iters; i++)
    MobilenetV1Manual1K_run(engine, &actions);
