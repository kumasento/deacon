...
struct Dfe {
  typedef MobilenetV1_actions_t dfe_run_actions_t;
  typedef MobilenetV1_dramRead_actions_t dram_read_actions_t;
  typedef MobilenetV1_dramWrite_actions_t dram_write_actions_t;

  static void ReadDRAM(max_engine_t *engine, dram_read_actions_t *actions) {
    MobilenetV1_dramRead_run(engine, actions);
  }
  static void WriteDRAM(max_engine_t *engine, dram_write_actions_t *actions) {
    MobilenetV1_dramWrite_run(engine, actions);
  }
};
...
int main(int argc, char *argv[]) {
  ...
  max_file_t *max_file = MobilenetV1_init();
  ...
  /* Launch run */
  MobilenetV1_actions_t actions;
  actions.param_batch_size = batch_size;

  LOG(INFO) << "Running ...\n";
  std::chrono::time_point<std::chrono::system_clock> start, end;
  start = std::chrono::system_clock::now();
  for (int i = 0; i < (int)num_iters; i++) MobilenetV1_run(engine, &actions);
