...
int main(int argc, char *argv[]) {
  ...
  uint64_t ofmap_num_elems =
      (conv1_H - conv1_K + 1) * (conv1_W - conv1_K + 1) * conv1_F * batch_size;
  ...
  for (uint64_t i = 0; i < ifmap_num_elems; i++) ifmap[i] = (rand() % 10) - 5;
  ...
  // conv1
  num_ops += (conv1_H - conv1_K + 1) * (conv1_W - conv1_K + 1) * conv1_C *
             conv1_F * conv1_K * conv1_K * 2;
