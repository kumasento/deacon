...
struct Dfe {
  typedef Resnet18Fst_actions_t dfe_run_actions_t;
  typedef Resnet18Fst_dramRead_actions_t dram_read_actions_t;
  typedef Resnet18Fst_dramWrite_actions_t dram_write_actions_t;

  static void ReadDRAM(max_engine_t *engine, dram_read_actions_t *actions) {
    Resnet18Fst_dramRead_run(engine, actions);
  }
  static void WriteDRAM(max_engine_t *engine, dram_write_actions_t *actions) {
    Resnet18Fst_dramWrite_run(engine, actions);
  }
};
...
int main(int argc, char *argv[]) {
  ...
  max_file_t *max_file = Resnet18Fst_init();
  ...
  for (int i = 0; i < (int)num_iters; i++)
    Resnet18Fst_run(engine, &actions);
