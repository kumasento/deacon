...
struct Dfe {
  typedef Resnet18OnnxB8_actions_t dfe_run_actions_t;
  typedef Resnet18OnnxB8_dramRead_actions_t dram_read_actions_t;
  typedef Resnet18OnnxB8_dramWrite_actions_t dram_write_actions_t;

  static void ReadDRAM(max_engine_t *engine, dram_read_actions_t *actions) {
    Resnet18OnnxB8_dramRead_run(engine, actions);
  }
  static void WriteDRAM(max_engine_t *engine, dram_write_actions_t *actions) {
    Resnet18OnnxB8_dramWrite_run(engine, actions);
  }
};
...
int main(int argc, char *argv[]) {
  ...
  max_file_t *max_file = Resnet18OnnxB8_init();
  max_engine_t *engine = max_load(max_file, "*");

  std::vector<ConvLayerParameters> cps;
  cps.push_back(ConvLayerParameters(max_file, "resnetv15conv0fwd"));
  cps.push_back(ConvLayerParameters(max_file, "resnetv15pool0fwd"));
  cps.push_back(ConvLayerParameters(max_file, "resnetv15stage1conv0fwd"));
  cps.push_back(ConvLayerParameters(max_file, "resnetv15stage1conv1fwd"));
  cps.push_back(ConvLayerParameters(max_file, "resnetv15stage1conv2fwd"));
  cps.push_back(ConvLayerParameters(max_file, "resnetv15stage1conv3fwd"));
  cps.push_back(ConvLayerParameters(max_file, "resnetv15stage2conv0fwd"));
  cps.push_back(ConvLayerParameters(max_file, "resnetv15stage2conv1fwd"));
  cps.push_back(ConvLayerParameters(max_file, "resnetv15stage2conv3fwd"));
  cps.push_back(ConvLayerParameters(max_file, "resnetv15stage2conv4fwd"));
  cps.push_back(ConvLayerParameters(max_file, "resnetv15stage3conv0fwd"));
  cps.push_back(ConvLayerParameters(max_file, "resnetv15stage3conv1fwd"));
  cps.push_back(ConvLayerParameters(max_file, "resnetv15stage3conv3fwd"));
  cps.push_back(ConvLayerParameters(max_file, "resnetv15stage3conv4fwd"));
  cps.push_back(ConvLayerParameters(max_file, "resnetv15stage4conv0fwd"));
  cps.push_back(ConvLayerParameters(max_file, "resnetv15stage4conv1fwd"));
  cps.push_back(ConvLayerParameters(max_file, "resnetv15stage4conv3fwd"));
  cps.push_back(ConvLayerParameters(max_file, "resnetv15stage4conv4fwd"));
  ...
  /* Launch run */
  Resnet18OnnxB8_actions_t actions;
  actions.param_batch_size = batch_size;

  LOG(INFO) << "Running ...\n";
  std::chrono::time_point<std::chrono::system_clock> start, end;
  start = std::chrono::system_clock::now();
  for (int i = 0; i < (int)num_iters; i++) Resnet18OnnxB8_run(engine, &actions);
