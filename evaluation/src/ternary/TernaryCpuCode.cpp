...
struct Dfe {
  typedef Ternary_actions_t dfe_run_actions_t;
  typedef Ternary_dramRead_actions_t dram_read_actions_t;
  typedef Ternary_dramWrite_actions_t dram_write_actions_t;

  static void ReadDRAM(max_engine_t *engine, dram_read_actions_t *actions) {
    Ternary_dramRead_run(engine, actions);
  }
  static void WriteDRAM(max_engine_t *engine, dram_write_actions_t *actions) {
    Ternary_dramWrite_run(engine, actions);
  }
};
...
int main(int argc, char *argv[]) {
  ...
  max_file_t *max_file = Ternary_init();
  ...
  auto weights_0 =
      CreateRandomArray<float>(cp0.F * cp0.C * cp0.K * cp0.K, -1., 1.0);
  ...
  for (int i = 0; i < weights_0_dfe.size(); ++i)
    weights_0_dfe[i] =
        weights_0[i] == 0
            ? 0
            : (weights_0[i] > 0
                   ? FloatToFixed<data_t>(1.0, cp0.dfe.num_frac_bits)
                   : FloatToFixed<data_t>(-1.0, cp0.dfe.num_frac_bits));
  ...
  for (int i = 0; i < weights_0_dfe.size(); ++i)
    weights_0_dfe[i] = weights_0[i] == 0 ? 0 : (weights_0[i] > 0 ? 1 : -1);

  Ternary_actions_t actions;
  ...
  // if (batch_size > 1) Ternary_run(engine, &actions);
  ...
  actions.param_batch_size = batch_size;
  start = std::chrono::system_clock::now();
  for (int i = 0; i < (int)num_iters; i++) Ternary_run(engine, &actions);
  end = std::chrono::system_clock::now();
  printf("Done\n");
