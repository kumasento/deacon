...
struct Dfe {
  typedef BottleneckShortcut_actions_t dfe_run_actions_t;
  typedef BottleneckShortcut_dramRead_actions_t dram_read_actions_t;
  typedef BottleneckShortcut_dramWrite_actions_t dram_write_actions_t;

  static void ReadDRAM(max_engine_t *engine, dram_read_actions_t *actions) {
    BottleneckShortcut_dramRead_run(engine, actions);
  }
  static void WriteDRAM(max_engine_t *engine, dram_write_actions_t *actions) {
    BottleneckShortcut_dramWrite_run(engine, actions);
  }
};
...
int main(int argc, char *argv[]) {
  ...
  max_file_t *max_file = BottleneckShortcut_init();
  ...
  std::vector<ConvLayerParameters> cps;
	...
	cps.push_back(ConvLayerParameters(max_file, "shortcut1"));
  ...
  /* Launch run */
  BottleneckShortcut_actions_t actions;
  ...
  for (int i = 0; i < (int)num_iters; i++)
    BottleneckShortcut_run(engine, &actions);
