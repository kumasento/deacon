...
int main(int argc, char *argv[]) {
  ...
  max_file_t *max_file = BinarisedConvLayer_init();
  ...
  int8_t *ifmap = (int8_t *) malloc(sizeof(int8_t) * ifmap_num_elems);
  int8_t *coeff_0 = (int8_t *) malloc(sizeof(int8_t) * coeff_0_num_elems);
  int8_t *ofmap = (int8_t *) malloc(sizeof(int8_t) * ofmap_num_elems);
  ...
  BinarisedConvLayer_actions_t actions;
  ...
  for (int i = 0; i < (int) num_iters; i ++)
    BinarisedConvLayer_run(engine, &actions);
