...
// maxeler
#include "test_lenet_fixed16_MaxDeep.h"
...
#define TEST_NAME "test_lenet_fixed16"
...
TEST(LeNetTest, MainTest) {
  ...
  test_lenet_fixed16_MaxDeep_actions_t actions;
  ...
  max_file_t *max_file = test_lenet_fixed16_MaxDeep_init();
  max_engine_t *engine = max_load(max_file, "local:*");

  LOG(INFO) << "Running test";
  std::chrono::time_point<std::chrono::system_clock> start, end;
  start = std::chrono::system_clock::now();
  test_lenet_fixed16_MaxDeep_run(engine, &actions);
